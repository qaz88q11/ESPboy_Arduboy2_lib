...
if not fxFlasher:
  filemenu.add_command(label = "Select Hex file", underline = 7, acccelerator = "Ctrl + H", command = selectHexFile)
filemenu.add_command(label = "Select Flash image", underline = 7, acccelerator = "Ctrl + F", command = selectFlashFile)
if not fxFlasher and not fxActivator:
  filemenu.add_command(label = "Select development data", underline = 0, acccelerator = "Ctrl + D", command = selectDevDataFile)
if not (fxActivator or fxFlasher):
  filemenu.add_separator()
  filemenu.add_command(label = "Backup EEPROM", underline = 0, acccelerator = "Ctrl + B", command = backupEEPROMThread)
  filemenu.add_command(label = "Restore EEPROM", underline = 0, acccelerator = "Ctrl + R", command = restoreEEPROMThread)
  filemenu.add_command(label = "View EEPROM", underline = 0, acccelerator = "Ctrl + L", command = viewEEPROMThread)
  filemenu.add_command(label = "Erase EEPROM", underline = 0, command = eraseEEPROMThread)
  filemenu.add_separator()
filemenu.add_command(label = "Exit", underline = 1, acccelerator = "Ctrl + X", command = root.quit)
